...
const LeaveRequestsPage: React.FC = () => {
  ...
  const { data: requests, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Admin/secretary: fetch all. Trainers: fetch own.
    ...
    // allow trainer to see own requests and admin/secretary to see all
    enabled: true,
  });
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div className="flex items-end">
                <Button onClick={() => createMutation.mutate()} disabled={createMutation.isLoading}>{createMutation.isLoading ? 'Sending...' : 'Submit Request'}</Button>
              </div>
                    ...
                    <td className="px-2 py-3">
                      {primaryRole !== 'trainer' && r.status === 'pending' && (
                        <div className="flex gap-2">
                          <Button onClick={() => reviewMutation.mutate({ id: r.id, status: 'approved' })}>Approve</Button>
                          <Button variant="destructive" onClick={() => reviewMutation.mutate({ id: r.id, status: 'rejected' })}>Reject</Button>
                        </div>
                      )}
                    </td>
                  </tr>
